...
class TransactionApiController extends Controller
{
    ...
    public function storeRental(Request $request)
    {
        $data = $request->validate([
            ...
            'satuan_sewa' => ['nullable', 'in:harian,bulanan'],
            'kena_ppn' => ['nullable', 'boolean'],
            'kena_pph' => ['nullable', 'boolean'],
        ...
        $kenaPpn = $data['kena_ppn'] ?? true;
        $kenaPph = $data['kena_pph'] ?? false;

        $subSewa = 0;
        $subOpr = 0;
        foreach ($data['items'] as $it) {
            $days = max(1, (int) ceil((strtotime($it['end_date']) - strtotime($it['start_date'])) / 86400));
            $qty = $satuan === 'bulanan' ? max(1, (int) ceil($days / 30)) : $days;
            // harga_sewa_unit = harga pokok sewa (di luar BBM/operator), per hari atau per bulan.
            $subSewa += (float) $it['harga_sewa_unit'] * $qty;
            $subOpr += (float) ($it['biaya_operator'] ?? 0) + (float) ($it['biaya_mobdemob'] ?? 0) + (float) ($it['biaya_bbm'] ?? 0);
        }
        $subtotal = $subSewa + $subOpr;
        $pajak = $kenaPpn ? round($subtotal * 0.11) : 0;       // PPN 11% keluaran
        $pph = $kenaPph ? round($subtotal * 0.02) : 0;          // PPh 23 2% dipotong pelanggan

        $sewa = DB::connection('voltra')->transaction(function () use ($data, $tid, $user, $subtotal, $pajak, $pph, $satuan, $kenaPpn, $kenaPph) {
        ...
        // Piutang dicatat NET dari PPh (pelanggan memotong PPh, jadi kas yang
        // akan diterima = tagihan - PPh). PPh dicatat sebagai aset kredit pajak.
        $lines = [
            ['kode_akun' => '1-1101', 'debit' => $subtotal + $pajak - $pph, 'keterangan' => 'Piutang ' . $sewa->no_invoice],
        ];
        if ($pph > 0) {
            $lines[] = ['kode_akun' => '1-1102', 'debit' => $pph, 'keterangan' => 'PPh 23 dipotong pelanggan (kredit pajak)'];
        }
        $lines[] = ['kode_akun' => '4-1001', 'kredit' => $subSewa, 'keterangan' => 'Pendapatan sewa'];
        if ($subOpr > 0) {
            $lines[] = ['kode_akun' => '4-1002', 'kredit' => $subOpr, 'keterangan' => 'Pendapatan operator & BBM'];
        }
        if ($pajak > 0) {
            $lines[] = ['kode_akun' => '2-2001', 'kredit' => $pajak, 'keterangan' => 'PPN 11% keluaran'];
        }
    ...
    public function updateRental(Request $request, int $id)
    {
        $user = $request->user();
        $tid = $user->id_perusahaan;
        $sewa = TransaksiSewa::where('id_perusahaan', $tid)->findOrFail($id);
        ...
        // Jurnal sewa terkait + cek periode masih aktif.
        $jurnal = JurnalAkuntansi::where('id_perusahaan', $tid)
            ->where('jenis_jurnal', 'sewa')
            ->where('referensi_tipe', 'transaksi_sewa')
        ...
        $satuan = $data['satuan_sewa'] ?? $sewa->satuan_sewa ?? 'harian';
        $kenaPpn = $data['kena_ppn'] ?? (bool) $sewa->kena_ppn;
        $kenaPph = $data['kena_pph'] ?? (bool) $sewa->kena_pph;

        // Hitung ulang dari detail unit yang sudah tersimpan.
        $subSewa = 0;
        $subOpr = 0;
        foreach (DetailSewa::where('id_sewa', $id)->get() as $dd) {
            $days = max(1, (int) ceil((strtotime($dd->end_date) - strtotime($dd->start_date)) / 86400));
            $qty = $satuan === 'bulanan' ? max(1, (int) ceil($days / 30)) : $days;
            $subSewa += (float) $dd->harga_sewa_unit * $qty;
            $subOpr += (float) $dd->biaya_operator + (float) $dd->biaya_mobdemob + (float) $dd->biaya_bbm;
        }
        $subtotal = $subSewa + $subOpr;
        $pajak = $kenaPpn ? round($subtotal * 0.11) : 0;
        $pph = $kenaPph ? round($subtotal * 0.02) : 0;
        ...
        $sewa->update([
            ...
            'pajak' => $pajak,
            'pph' => $pph,
            'tgl_jatuh_tempo' => $data['tgl_jatuh_tempo'] ?? $sewa->tgl_jatuh_tempo,
            'keterangan' => $data['keterangan'] ?? $sewa->keterangan,
        ]);

        // Tulis ulang baris jurnal sewa agar konsisten dengan tagihan baru.
        if ($jurnal) {
            $lines = [
                ['kode_akun' => '1-1101', 'debit' => $subtotal + $pajak - $pph, 'keterangan' => 'Piutang ' . $sewa->no_invoice],
            ];
            ...
            try {
                $this->journal->replaceLines($jurnal, $lines, 'Terbit Invoice ' . $sewa->no_invoice);
        ...
        return response()->json([
            'message' => 'Invoice ' . $sewa->no_invoice . ' diperbarui.',
            'sewa' => $sewa->refresh(),
        ]);
    }
    ...
    public function addPartToMaintenance(Request $request, int $id)
    {
        ...
        // Catat pemakaian + potong stok dalam satu transaksi (atomik).
        $qtyAdd = (int) $data['qty_digunakan'];
        $row = DB::connection('voltra')->transaction(function () use ($id, $data, $part, $qtyAdd) {
            $existing = DetailPemeliharaan::where('id_pemeliharaan', $id)->where('id_part', $data['id_part'])->first();
            if ($existing) {
                $newQty = $existing->qty_digunakan + $qtyAdd;
                $existing->update([
                ...
                $row = DetailPemeliharaan::create([
                    ...
                    'qty_digunakan' => $qtyAdd,
                    'subtotal_harga_part' => $qtyAdd * (float) $part->harga_satuan,
                ]);
            }
            SukuCadang::where('id_part', $data['id_part'])->decrement('stok_tersedia', $qtyAdd);

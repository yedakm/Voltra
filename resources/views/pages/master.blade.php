...
@elseif ($kind === 'brands')
    ...
    <x-section-header title="Merek Genset"
        subtitle="Daftar merek genset — berlaku untuk semua perusahaan">
        <x-slot:actions>
            <button class="btn btn-primary" @click="createOpen = true"><x-icon name="plus" :size="14" /> Merek Baru</button>
        </x-slot:actions>
    </x-section-header>
    <div class="card p-4 mb-3 flex items-center gap-3" style="background:#f3eefd;border-color:#d9c7f5">
        <div style="color:#5b2ea8"><x-icon name="tag" :size="18" /></div>
        <div class="text-[12.5px]" style="color:#5b2ea8">
            <span class="font-medium">Hanya Super Admin yang dapat menambah merek baru.</span> Daftar merek dipakai bersama agar setiap perusahaan tidak perlu mendaftarkan ulang.
        </div>
    </div>
        ...
        <table class="w-full text-[13px]">
            <thead>
                <tr class="bg-ink-50 border-b border-ink-200 text-ink-500 text-[11px] uppercase tracking-wider">
                    ...
                    <th class="px-3 py-2.5 text-right font-semibold">Total Unit (semua tenant)</th>
                </tr>
            </thead>
            <tbody>
